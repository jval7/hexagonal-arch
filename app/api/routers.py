...
def generate_answer(query: str, rag_service: usecases.RAGService = Depends(dependencies.get_rag_service)):
    return {"answer": rag_service.generate_answer(query)}


@rag_router.post("/save-document/")
def save_document(document: DocumentInput, rag_service: usecases.RAGService = Depends(dependencies.get_rag_service)):
